...
import { FileTextIcon, FolderIcon, FolderOpenIcon, ChevronRightIcon, ChevronDownIcon } from '@components/common/ui/icons';
import { TreeNodeMenu } from './TreeNodeMenu';
import { useTreeNodeEditing } from './useTreeNodeEditing';
...
export const TreeNode: React.FC<TreeNodeProps> = observer(({ node, level, collapsed, currentNoteId, onSelectNote }) => {
  const sidebarStore = useSidebarStore();
  const navigate = useNavigate();
  const [showDropdown, setShowDropdown] = useState(false);
  const dropdownRef = useRef<HTMLDivElement>(null);
  const { inputRef, inputValue, setInputValue, isEditing, editingMode, handleKeyDown, handleBlur } = useTreeNodeEditing(node);
